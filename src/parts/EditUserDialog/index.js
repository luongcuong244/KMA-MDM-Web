...
import styles from "./edit_user_dialog.module.scss";
import clsx from "clsx";
import userService from "../../services/user.service";

const EditUserDialog = ({ user, isOpen, onClose, onSubmit }) => {
    const [isExiting, setIsExiting] = useState(false);
    const [error, setError] = useState("");
    const [userName, setUserName] = useState(user.userName || "");
    const [maxDevices, setMaxDevices] = useState(user.maxManagedDevices || 10);
    ...
    return (
        <div className={clsx(styles.dialogBackdrop, isExiting && styles.dialogBackdropExit)}>
            <div className={clsx(styles.dialog, isExiting ? styles.dialogExit : styles.dialogEnter)}>
                <label className={styles.title}>Sửa thông tin quản lý</label>
                {
